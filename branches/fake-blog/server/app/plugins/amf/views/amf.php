	...
	function render($action = null, $layout = null, $file = null) {
		...
		if (!empty($this->body)) {
			...
			if (!empty($this->viewVars['Error'])) {
				$response = 'onStatus';
			} elseif (!empty($this->viewVars['Responder'])) {
				$response = $this->viewVars['Responder'];
				unset($this->viewVars['Responder']);
			}
...
class AmfStream {
	...
	/**
	 * Returns the length of the stream contents.
	 *
	 * @return integer
	 */
	function length() {
		$_this =& AmfStream::getInstance();
		return strlen($_this->__stream);
	}
