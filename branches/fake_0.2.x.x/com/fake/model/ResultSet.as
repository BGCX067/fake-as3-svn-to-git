/* SVN FILE: $Id: ResultSet.as 154 2008-08-30 18:16:40Z gwoo.cakephp $ */
/**
 * Description
 *
 * Fake
 ...
 * @lastmodified	$Date: 2008-08-31 01:16:40 +0700 (Sun, 31 Aug 2008) $
		...
		protected function _format(source:Object, prevKey:* = null):*
		{
			...
			for (var key:* in source)
			{
				...
				else if (value is Array && value.length > 0)
				{
					...
					{
						var model:Model = Model.construct(key);
						if (model)
						{
				...
				else if (key is Number)
				{
					if (prevKey)
					{
						prevKey = Inflector.singularize(prevKey);
					}
					if (value.hasOwnProperty(prevKey))
					{
						array.push(_format(value, prevKey));
					}
					else
					{
						model = Model.construct(prevKey, value);
						if (model)
						{
							array.push(model);
						}
